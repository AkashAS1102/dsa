...
import { PROBLEMS, RECENT_SUBMISSIONS } from '@/lib/mock-data';
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="relative overflow-hidden rounded-3xl gradient-teal p-6 text-white shadow-card-lg">
        ...
        <div className="relative flex items-center justify-between">
          <div>
            ...
            <h2 className="text-2xl font-extrabold mt-0.5">{summary.name}</h2>
            <p className="text-sm text-teal-200 mt-1">
              You&apos;re ranked <span className="text-white font-bold">#{summary.rank}</span> on the leaderboard.
              Keep it up.
            </p>
            <div className="flex items-center gap-4 mt-4">
              <div className="flex items-center gap-2 bg-white/15 rounded-xl px-3 py-1.5">
                <Flame className="w-4 h-4 text-orange-300" />
                <span className="text-sm font-bold">{summary.streak} day streak</span>
              </div>
              <div className="flex items-center gap-2 bg-white/15 rounded-xl px-3 py-1.5">
                <Star className="w-4 h-4 text-yellow-300" />
                <span className="text-sm font-bold">{summary.points.toLocaleString('en-US')} pts</span>
              </div>
            </div>
          </div>
          <img
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Problems Solved" value={summary.totalSolved} icon={CheckCircle2} color="bg-brand-900" sub={`of ${totalProblems} total`} />
        <StatCard label="Current Rank" value={`#${summary.rank}`} icon={Trophy} color="bg-yellow-500" sub="Leaderboard position" />
        <StatCard label="Total Points" value={summary.points.toLocaleString('en-US')} icon={Zap} color="bg-violet-500" sub="Earned from submissions" />
        <StatCard label="Day Streak" value={summary.streak} icon={Flame} color="bg-orange-500" sub="Keep it going!" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-card ring-1 ring-gray-100 p-5">
          ...
          <div className="space-y-2">
            {RECENT_SUBMISSIONS.slice(0, 5).map((submission) => {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-xs text-gray-400 font-mono uppercase">{submission.language}</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="space-y-3">
          <div className="bg-white rounded-2xl shadow-card ring-1 ring-gray-100 p-5">
            ...
            <div className="space-y-2">
              <Link href="/problems" className="flex items-center justify-between p-3 rounded-xl bg-brand-50 hover:bg-brand-100 transition-colors group" id="quick-start-coding">
                <div className="flex items-center gap-3">
              ...
              <Link href="/visualizer" className="flex items-center justify-between p-3 rounded-xl bg-violet-50 hover:bg-violet-100 transition-colors group" id="quick-visualizer">
                <div className="flex items-center gap-3">
              ...
              <Link href="/leaderboard" className="flex items-center justify-between p-3 rounded-xl bg-yellow-50 hover:bg-yellow-100 transition-colors group" id="quick-leaderboard">
                <div className="flex items-center gap-3">
          ...
          <div className="bg-white rounded-2xl shadow-card ring-1 ring-gray-100 p-5">
            ...
            <div className="space-y-2">
              {[
                { label: 'Easy', count: easyProblems, color: 'bg-green-500', solved: summary.solvedEasy },
                { label: 'Medium', count: mediumProblems, color: 'bg-yellow-500', solved: summary.solvedMedium },
                { label: 'Hard', count: hardProblems, color: 'bg-red-500', solved: summary.solvedHard },
                ...
                <div key={label} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className={`h-full ${color} rounded-full transition-all`} style={{ width: `${(solved / count) * 100}%` }} />
                    </div>
                  </div>
                </div>
              ))}
